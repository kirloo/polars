            ...
            .map(|((((((y, m), d), h), mnt), s), ns)| {
                    ...
                    (y, m, d, h, mnt, s, ns)
                {
                    NaiveDate::from_ymd_opt(y, m as u32, d as u32).map_or_else(
                        ...
                        || if strict { Err(polars_err!(ComputeError: format!("Invalid date components ({}, {}, {}) supplied", y, m, d))) } else { Ok(None) }, 
                        // We have a valid date.
                        |date| {
                            date.and_hms_nano_opt(h as u32, mnt as u32, s as u32, ns as u32)
                                .map_or_else(
                                    // We have invalid time components for the specified date.
                                    || if strict { Err(polars_err!(ComputeError: format!("Invalid time components ({}, {}, {}, {}) supplied", h, mnt, s, ns))) } else { Ok(None) },
